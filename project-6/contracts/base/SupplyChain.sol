...
//contract SupplyChain is Ownable, Secondary {
    ...
    function kill() public onlyOwner {
        address payable owner_addr = address(uint160(owner()));
    ...
    // Define a function 'sortItem' that allows a farmer to mark an item 'Sorted'
    function sortItem(uint _upc) public harvested(_upc) verifyCaller(items[_upc].ownerID) {
        items[_upc].itemState = State.Sorted;
        emit Sorted(_upc);
    }

    // Define a function 'packItem' that allows a farmer to mark an item 'Packed'
    function packItem(uint _upc) public sorted(_upc) verifyCaller(items[_upc].ownerID) {
    ...
    function buyItem(uint _upc) public payable forSale(_upc) paidEnough(items[_upc].productPrice) checkValue(_upc) onlyWholesaler {
        ...
        // Update the appropriate fields - ownerID, wholesalerID, itemState
        items[_upc].ownerID      = msg.sender;
        items[_upc].wholesalerID = msg.sender;
    ...
    function purchaseItem(uint _upc) public payable received(_upc) paidEnough(items[_upc].productPrice) checkValue(_upc) {
    ...
    function fetchItemBufferTwo(uint _upc) public view returns (uint          itemSKU,
        ...
        wholesalerID = items[_upc].wholesalerID;
